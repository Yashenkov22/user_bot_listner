import re
import asyncio
import pytz

from datetime import timedelta, datetime

from telethon import TelegramClient, events
from telethon import types

api_id = 25430288
api_hash = "08616908140d6688f2e0107805741d9d"
# session_name = "my_userbot"

loop = asyncio.get_event_loop()

client = TelegramClient('./telethon_userbot.session',
                        api_id=api_id,
                        api_hash=api_hash,
                        loop=loop)

moscow_tizone = pytz.timezone('Europe/Moscow')


# PATTERNS = [
#     r"\bобмен\w*\b",
#     r"\bкурс\w*\b",
#     r"\bвалют\w*\b",
#     r"\bденег\b",
#     r"\bденьг\w*\b",
#     r"\bналич\w*\b",
#     # r"\bскрининг\w*\b",
# ]
PATTERNS = {
    # 'обмен': r"\bобмен\w*\b",
    # 'курс': r"\bкурс\w*\b",
    # 'валют': r"\bвалют\w*\b",
    # 'деньги': r"\bденьги\w*\b",
    # 'наличные': r"\bналичные\w*\b",
    '': '',
    'Где обменять': r'Где обменять',
    'лучше обменять': r'лучше обменять',
    'обмен валют': r'обмен валют',
    'обмен денег': r'обмен денег',
    'обмен наличных': r'обмен наличных',
    'обмен наличные': r'обмен наличные',
    'наличные обмен': r'наличные обмен',
    'наличных обмен': r'наличных обмен',
    'обмен крипта': r'обмен крипта',
    'обмен крипты': r'обмен крипты',
    'крипта обмен': r'крипта обмен',
    'выгодный курс': r'выгодный курс',
    'курс выгодный': r'курс выгодный',
    'курс для обмена': r'курс для обмена',
    'курс для валют': r'курс для валют',
    'курс валют': r'курс валют',
    'валют курс': r'валют курс',
    'курс денег': r'курс денег',
    'денег курс': r'денег курс',
    'курс для денег': r'курс для денег',
    'курс наличных': r'курс наличных',
    'курс наличные': r'курс наличные',
    'наличные курс': r'наличные курс',
    'курс для наличных': r'курс для наличных',
    'курс безнал': r'курс безнал',
    'безнал курс': r'безнал курс',
    'курс безналичных': r'курс безналичных',
    'безналичные курс': r'безналичные курс',
    'курс для безнал': r'курс для безнал',
    'курс для безналичных': r'курс для безналичных',
    'курс крипты': r'курс крипты',
    'крипта курс': r'крипта курс',
    'курс юсдт': r'курс юсдт',
    'юсдт курс': r'юсдт курс',
    'курс usdt': r'курс usdt',
    'usdt курс': r'usdt курс',
    'курс для крипты': r'курс для крипты',
    'курс для юсдт': r'курс для юсдт',
    'курс для usdt': r'курс для usdt',
    'посоветуйте обменник': r'посоветуйте обменник',
    'наличные на': r'наличные на',
    'рубли на': r'рубли на',
    'выгодный обменник': r'выгодный обменник',

}

for key in PATTERNS:
    PATTERNS[key] = re.compile(PATTERNS[key], re.IGNORECASE)

# BLACK_LIST_PATTERNS = (
#     r"\bпиши в личку\w*\b",
#     r"\bпиши в лc\w*\b",
#     r"\bпишите в личные сообщения\w*\b",
#     r"\bвыкупаем\w*\b",
#     r"\bбуду благодарен за репост\w*\b",
#     r"\bжду вас в – лс\w*\b",
#     r"\bпиши мне \"+\"\w*\b",
#     r"\bкупим ваш\w*\b",
#     r"\bкурс вам понравится\w*\b",
#     r"\bлучший курс\w*\b",

# )


BLACK_PATTERN = (
    r"пиши в личку",
    r"пиши в (лс|лc)",
    r"пишите в личные сообщения",
    r"выкупаем",
    r"буду благодарен за репост",
    r"жду вас в\s*–?\s*лс",
    r'пиши мне\s*"\+"',
    r"купим ваш",
    r"курс вам понравится",
    r"пишите в лс",
    r"пишите в личку",
    r"скупаю юсдт",
    r"скамерам не писать",
    r"ищу обмен",
    r"личная встреча",
    r"офис или встреча",
    r"tether",
    r"личка открыта",
    r"в лс",
    r"поможем с",
    r"пиcать в (лс|лc)",
    r"доставка по городу",
    r"exchange",
    r"кэш",
    r"трахаюсь",
    r"секс",
    r"при личной встрече",
    r"прибыль",
    r"по хорошей цене",
    r"выкупаю",
    r"сможет продать",

    r"обмен крuпты лично",
    r"выгoдно и безопасно",
    r"пиcать в лс",
    r"приобретаю usdt",
    r"консультация",
    r"срочно покупаем usdt",
    r"интересует usdt",
    r"всё при встрече",
    r"предлагаю хороший курс",
    r"работаю по всему миру",
    r"только встреча",
    r"в курсе",
    r"хорошей репутацией",
    r"куплю usdt за наличные",
    r"покупка usdt за наличные",
    r"реальная встреча",
    r"бесплатное обучение",
    r"идет набор команды",
    r"криптожара",
    r"индивидуальный подход",

    r"покупаю usdt с реальными курсами",
    r"без скрытых комиссий",
    r"без обмана и предоплат",
    r"пишите, обсудим",
    r"whatsapp",
    r"куплю юсдт за наличные",
    r"хочешь обменять usdt",
    r"связь в (л\.с\.|л\.c\.)",

    r"самые низкие цены на рынке",
    r"стол в стол",
    r"курс договоримся",
    r"рассматриваю покупку usdt",
    r"скупаем usdt",
    r"без предоплат и комиссий",
    r"могу выдать наличные",

    r"сделаем все быстро",
    r"перевод без комиссии",
    r"обменяю юсдт на наличку",
    r"доставка бесплатно",
    r"курс зависит от обьема",
    r"никаких переплат",
    r"круглосуточно и без выходных",

    r"для обмена пишите оператору",
    r"покупаю юсдт по адекватному курсу",
    r"готов на обмен",
    r"работаю 24/7",
    r"работаем быстро",
    r"купить юсдт по хорошему курсу",

    r"курсы английского языка",
    # r"моментально",
    r"работаем в любом городе",
    r"плачу налом за тезер",
    r"f-1",
    r"языковые курсы",
    r"зaрaбoтoк удaленнo",
    r"покупаю usdt и usdc",

    r"пиши — всё обсудим",
    r"продажа — по лучшему курсу",
    r"Принимаем usdt",
    r"стол на стол",
    r"нужен обмен",
    r"работаем в любом городе",
    r"сапборд",
    r"сап борд",
    r"плачу налом за тезер",
    r"встречусь лично",
    r"курс приятный",

    r"скупаю",
    r"нужен обмен",
    r"нужен юсдт",
    r"конвертор",
    r"золотая корона",
    r"гарантируем анонимность",
    r"аренда",
    r"сотрудничеств\w*\b",
    r"старого образца",

    r"старый образец",
    r"лучшие цены",
    r"работаю честно",

    r"проверенный обмен",
    r"обмен\/thb\/таиланд",
    r"встреча в городе",
    r"ищу возможность купить",
    r"наличные прямо сейчас",
    r"с обменниками не работаю",
    r"лично встречусь",
    r"оформим за вас",
    r"водительских удостоверений",
    r"обменяю вашу",
    r"писать в личку",
    r"курс обсуждаем",
    r"подъеду",
    r"набирайте",
    r"договоримся",
    r"быстрый обмен",
    r"на ваши",
    r"прямо сейчас",
    r"взаимовыгодному",
    r"менеджер",
    r"покупаю для себя",
    r"живая встреча",
    r"доставка наличных",
    r"надёжно и лично",
    r"встреча в городе",
    r"работа",
    r"старые",
    r"встреча лично",
    r"при встрече",
    r"пишите обсудим",

    r"срочно нужен",
    r"права",
    r"детали обсудим",
    r"личной встречи",
    r"предлагайте курс",
    r"партнеров",
    r"постоянный обмен",

    r"личные встречи",
    r"покупаю",
    r"куплю",

    r"скам",
    r"fleetli",
    r"ищу продавцов",
    r"брать с собой",
    r"срочно",
    r"немного",
    r"прозрачный",
    r"приобрету",
    r"свежему",
    r"в личку",
    r"конфиденциальный",
    r"приятно удивит",
    r"в удобном месте",
    r"заявку",

    r"заявка",
    r"ваш",
    r"без опыта",
    r"продайте",
    r"кто обменивает usdt",
    r"дам больше",
    r"старого",
    r"образца",
    r"не обижу",
    r"срочный набор",
    r"в любое время",

    r"чистая сделка",
    r"условиям по запросу",
    r"дам больше",
    r"срочность",
    r"не обижу",
    r"процент",
    r"доход от",
    r"Пиши «\+»",
    r"поддержка 24\/7",
    r"заберу",
    r"принимаем",
    r"направления",
    r"есть у кого",
    r"связь",
    r"жми сюда",
    r"btrubt",
    r"персонал",

    r"CRYPIK",
    r"Можем обменять на",
    r"пишите мне",
    r"в личных сообщениях",
    r"Покупка USDT",
    r"Выдам",
    r"Быстрые деньги",
    r"Жми кнопку",
    r"Закупаю юсдт",
    r"Пишите встретимся",
    r"Дам хороший",
    r"Минимальная сумма обмена",
    r"Беру USDT",

    r'πиcaть ⲃ лc',
    #
    r"[πΠпП]и[cс][аa][тt][ьbь] [ⲃвb][ ]?[лЛ][cсCС]",
    #
    r"мошенники не беспокойте",
    r"решил продать",
    r"\% накину хороший",
    r"аптечк(у|а)",
    r"24\/7",
    r"24 на 7",
    r"доставка до вас",
    r"kpиптo",
    r"новым клиентам",
    r"Курс динамичный",
    r"Подработка",
    r"согласования сделки",
    r"USDT за наличные",
    r"вс(ё|е) сделаем быстро",
    r"прозрачные условия",
    r"гарантирую",
    r"Отвечаю быстро",
    r"конфиденциально",
    r"Встреча или офис",
    r"взаймы",
    r'"\+"',
    r"«\+»",

    r"косметолог",
    r"процедур",
    r"старых",
    r"Ищу человека",
    r"в личные",
    r"под залог",
    r"Покупаем",
    r"Продаем",
    r"Продаём",
    r"2003",
    r"2006",
    r"серую крипту",
    r"Обменяю наличные на",
    r"Можем встретиться",
    r"Мне нужны",
    r"жду от вас предложений",
    r"нужна крипта",
    r"обмениваем крипту",
    r"Кто продает юсдт",
    r"Дайвинг",
    r"старых",

    r"обращайтесь",
    r"курс выше рынка",
    r"Встреча в удобном",
    r"кто то продает",
    r"2009",
    r"пара часов времени",
    r"KoнՓиденциaльнo",
    r"пишите ⲃ личʜыe сoобщеʜия",
    r"ΠOKᎩΠAЮ И ПPOДАЮ KРИΠTОBАЛЮТУ",
    r"Свободное время",
    r"санкции",
    r"Осторожно, разводит",
    r"Оплата инвойсов",
    r"без всяких мошенников",
    r"Нал\/безнал",
    r"Наши Отзывы",
    r"FAT_TONY",
    r"к Тони",
    r"ДOΠЛАТA ЗA ОƃMΕH",
    r"большие деньги",

    r"SPRINGFIELD CHANGE",
    r"Фиксированный курс",
    r"арендовать",
    r"аренда",
    r"ⲃ личныe cообщeʜия",
    r"ПОКᎩПАЮ И ΠРOДAЮ",
    r"Напишите нам",
    r"Гибκий κурc",
    r"КoнՓидeʜциальʜо",
    r"популярные монеты",
    r"По всем вопросам",
    r"Быстрая выдача",
    r"Безопасные сделки",

    r"Покупка/Продажа",
    r"к вам лично",
    r"таро",
    r"ЭФИР",
    r"ⲃ личʜыe cообщeния",
    r"пишите ⲃ личʜыe",
    r"уточним детали",
    r"МАССАЖ",
    r"Перестановки",
    r"в закрепе",
    r"фотками",
    r"фотки",
    r"скрытых условий",
    r"Моментальный расчёт",
    r"гарантируем",
    r"удобной форме",
    r"резина",
    r"резину",
    r"обналичу",
    r"быстрo обyчим",
    r"в личке",
    r"Гибⲕий κурс",
    r"без задержек",
    r"биржа",
    r"хочешь зарабатывать",
    r"встретится лично",

    r"CryptoPoint",
    r"Молниеносный обмен",
    r"СКИДКА",
    r"Объёмы",
    r"ДOПЛАTA ЗA ОБMΕН",
    r"ⲃ личныe сообщеʜия",
    r"выезд к вам",
    r"соблюдение",
    r"сотрудничать",
    r"лишних вопросов",
    r"в приватные сообщения",
    r"ПOКᎩΠAЮ И ПPОДAЮ",

    r"обсудим курс",
    r"любые условия",
    r"кто заинтересован",
    r"юсд.т",
    r"кто продаёт",
    r"Честные курсы",
    r"NodeX",
    r"Приобретаем",
    r"Продажа",
    r"Легкие деньги",
    r"СНЯТИЕ",
    r"UnionPay",
    r"ΠРOДAЮ KРИΠТОBАЛЮТУ",
    r"KоʜՓидeнциaльнo",
    r"личныe сообщeния",
    r"quantum_swap",
    r"офис или курьер",
    r"без посредников",
    r"моментальные расчёты",
    r"Напиши нам",
    r"мы подстроимся",

    r"Нужен человек",
    r"БУДЬТЕ ОСТРОЖНЫ",
    r"АНГЛІЙСЬКОЇ мови",
    r"Никаких акций",
    r"Никаких заманух",
    r"не подлежит",
    r"ex24pro_bot",
    r"сво",
    r"займы",
    r"Пишите",
    r"бизнес-тренер",
    r"Bkk",
    r"Быстро и прозрачно",
    r"заявке",
    r"заявка",
    r"продаю",
    r"продам",
    r"Криптозайм",
    r"займ",
    r"перестановка",
    r"проект",
    r"нужны",
    r"повышенный курс",
    r"U S D T",
    r"тизер",

    r"Обменю",
    r"маршрут",
    r"Crypto",
    r"Point",
    r"Продлил",
    r"ориентировочно",
    r"пиши нам",
    r"Change",
    r"груз",
    r"u.s.d.t",
    r"Другие валюты",

    r"помочь",
    r"валюті",
    r"грн",
    r"Ищете",
    r"специальный",
    r"прмокод",
    r"флирт",
    r"Flinra",
    r"анонимно",
    r"сбор",
    r"доплатой",
    r"NovaEx",
    r"легкие",

)

BLACK_LIST_PATTERNS = [re.compile(p, re.IGNORECASE) for p in BLACK_PATTERN]


@client.on(events.NewMessage)
async def listen_any_message(event):
    asyncio.create_task(process_event(event))


async def process_event(event):
    text = event.raw_text.lower()
    # async for dialog in client.iter_dialogs():
    #     print(f"Название: {dialog.name} | ID: {dialog.id} | Тип: {type(dialog.entity)}")
    # msg = event.message

    if not event.sender or event.sender.bot:
        return

    chat_to = '@test_userbot22'

    for pattern_word, pattern in PATTERNS.items():
        try:
            # print(f'проверяю {pattern_word}...')
            if re.search(pattern, text):
                # black list words
                if any(re.search(black_pattern, text) for black_pattern in BLACK_LIST_PATTERNS):
                    print('black pattern!')
                    return
                # print('успешно')

                user_data = event.sender

            # print('user', user_data)
                chat: types.Channel = event.chat

                print('chat', chat.__dict__)

                chat_username = chat.username
                chat_name = chat.title

                try:
                    reply_to = event.message.reply_to
                    # print('REPLY TO ', reply_to)
                    topic_id = reply_to.reply_to_top_id

                    if topic_id is None:
                        raise Exception()
                    
                except Exception as ex:
                    print(ex)
                    try:
                        topic_id = reply_to.reply_to_msg_id
                    except Exception as ex:
                        print(ex)
                        topic_id = None

            # print('topic_id',topic_id)

                message_date = event.date + timedelta(hours=3)

                name = None

                if user_data:
                    username = user_data.username
                    first_name = user_data.first_name
                    last_name = user_data.last_name

                    name = username or first_name or last_name or f'Имя неизвестно, id {user_data.id}'

                if name is None:
                    name = f'Имя неизвестно'

                print(f'***\nСообщение из {chat_name}\n\nВремя: {datetime.now(tz=moscow_tizone)}\n***\n')

                user_id_link = f'tg://user?id={user_data.id}'

                _text = f'🔔 Сообщение из чата <a href="https://t.me/{chat_username}">{chat_name}</a>\n\nНайдено совпадение по ключевому слову: {pattern_word}\n\nТекст сообщения:\n<pre>\n{event.raw_text}</pre>\n\nПользователь: {name}\nВремя сообщения: {message_date.strftime("%d.%m.%Y %H:%M")}\n\nСсылки на пользователя:\n- по id: {user_id_link}'
                
                if user_data.username:
                    username_link = f'https://t.me/{user_data.username}'
                    _text += f'\n- по username: {username_link}'

                if topic_id:
                    url = f"https://t.me/{chat_username}/{topic_id}/{event.message.id}"
                else:
                    url = f"https://t.me/{chat_username}/{event.message.id}"

                _text += f'\n\n<a href="{url}"><b>Перейти к сообщению</b></a>'

                await client.send_message(chat_to,
                                        _text,
                                        parse_mode='HTML',
                                        link_preview=False)
                break
        except Exception as ex:
            print(ex)
            continue


client.start()

print('--- START USER BOT...')

client.run_until_disconnected()
